	...
	<body>
		...
		<form action="novavijest.php" method="post" id="novavijest" class="kontakt" enctype="multipart/form-data">
			<fieldset>
				...
				<input type="tekst" name="autor" required onkeyup="autorProv()" disabled value="<?php echo $_SESSION['name']; ?>"><br><br>
		...
		<?php 
			...
			if( isset($_POST['submit'])  )
			{
				...
				$autor=htmlEntities($_SESSION['name'], ENT_QUOTES);
				$tekst=htmlEntities($_POST['text'], ENT_QUOTES);
				$vrijeme= date('m.d.Y H:i:s');
				$slika = $_FILES['image']['name'];
				$tmp_slika =$_FILES['image']['tmp_name'];
				
				$iskaz = "insert into vijesti (naslov,autor,tekst,slika,vrijeme) values (\"$naslov\",\"$autor\",\"$tekst\",\"$slika\",\"$vrijeme\")";
				if(!mysqli_query($con,$iskaz)) {
